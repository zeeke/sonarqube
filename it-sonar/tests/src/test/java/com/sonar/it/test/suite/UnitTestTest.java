...
public class UnitTestTest {
  ...
  public void shouldReuseCoberturaAndSurefireReports() {
    MavenBuild build = MavenBuild.builder()
      .setPom(ItUtils.locateProjectPom("test/reuse-cobertura-and-surefire-reports"))
      .addGoal("clean install") // cobertura and surefire are executed during build
      .build();
    ...
    MavenBuild analysis = MavenBuild.builder()
      ...
      .withProperty("sonar.dynamicAnalysis", "reuseReports") // IMPORTANT, otherwise cobertura will be executed again
      .build();
    orchestrator.executeBuilds(build, analysis);

    Resource project = orchestrator.getServer().getWsClient().find(ResourceQuery.createForMetrics("com.sonarsource.it.samples:reuse-cobertura-and-surefire-reports",
      "test_success_density", "test_failures", "test_errors", "tests", "skipped_tests", "test_execution_time", "coverage"));
  ...
  @Test
  public void shouldSupportDeprecatedPluginProperty() {
    MavenBuild analysis = MavenBuild.builder()
      .setPom(ItUtils.locateProjectPom("test/with-tests"))
      .addSonarGoal()
      .withProperty("sonar.core.codeCoveragePlugin", "cobertura")
      .build();
    BuildResult result = orchestrator.executeBuild(analysis);
    assertThat(result.getLogs()).contains("Cobertura");
    assertThat(result.getLogs()).doesNotContain("JaCoCo");
  }
