...
public class PurgeTest {
  ...
  public void should_keep_only_one_snapshot_per_day() {
    ...
    extraProps.put("sonar.dbcleaner.hoursBeforeKeepingOnlyOneSnapshotByDay", "0");
    scan("shared/struts-1.3.9-diet", extraProps);
  ...
  public void should_delete_removed_modules() {
    ...
    extraProps.put("sonar.dbcleaner.hoursBeforeKeepingOnlyOneSnapshotByDay", "0");
    scan("purge/modules/after", extraProps);
  ...
  /**
   * SONAR-3120 - see the comment from Freddy
   */
  @Test
  public void should_support_analysis_of_module_then_project() {

    scan("shared/multi-modules-sample/module_a");
    assertSingleSnapshot("com.sonarsource.it.samples:module_a1:com.sonar.it.samples.modules.a1.HelloA1");

    scan("shared/multi-modules-sample");
    // past file snapshots of module_a have been deleted
    assertSingleSnapshot("com.sonarsource.it.samples:module_a1:com.sonar.it.samples.modules.a1.HelloA1");
  }
  ...
  public void should_not_delete_historical_data_of_directories() {
    ...
    MavenBuild.Builder builder = MavenBuild.builder()
        ...
        .addSonarGoal()
        .withProperty("sonar.projectDate", "2012-02-02").withProperty("sonar.dbcleaner.cleanDirectory", "false");
    orchestrator.executeBuild(builder.build());
  ...
  private MavenBuild scan(String path) {
    return scan(path, (Properties) null);
  }

  private MavenBuild scan(String path, String date) {
    Properties props = new Properties();
    props.put("sonar.projectDate", date);
    return scan(path, props);
  }

  private MavenBuild scan(String path, Properties extraProperties) {
    MavenBuild.Builder builder = MavenBuild.builder()
